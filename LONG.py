...
def find_startnode(overlap_d):
    ...
    for key in overlap_d.keys:
        if key not in overlap_d.values():
            start_node=key
    return start_node


def layout_set(overlap):
    """
    :param overlap: the overlap list generated by graph
    :return: the order of fasta reads
    """
    read_order=[]
    for n, pair in enumerate(overlap):
        primer_5, primer_3=pair
        if n==0:  # for the first
            read_order.append(primer_5)
            read_order.append(primer_3)
        else:
            if primer_5 in read_order and primer_3 not in read_order:
                read_order.append(primer_3)
            elif primer_3 in read_order and primer_5 not in read_order:
                read_order.insert(0,primer_5)
            elif primer_3 in read_order and primer_5 in read_order:
                print "Both in error, check the data {0}, {1}".format(primer_5, primer_3)
            elif primer_3 not in read_order and primer_5 not in read_order:
                print "Both out error, check the data {0}, {1}".format(primer_5, primer_3)
    print read_order
    return read_order

def layout(overlap_d):
    overlap=[]
    for k, v in overlap_d.iteritems():
        overlap.append((k,v.keys()[0]))

    print overlap
    return layout_set(overlap)
